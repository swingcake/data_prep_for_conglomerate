...
for datum in data:
    if datum.find("field", {'name':"sbi.offers.title"}) == None:
        title = ""
    else:
        title = datum.find("field", {'name':"sbi.offers.title"}).text

    if datum.find("field", {'name':"sbi.offers.description"}) == None:
        description = ""
    else:
        description = datum.find("field", {'name':"sbi.offers.description"}).text

    if datum.find("field", {'name':"sbi.offers.offertext"}) == None:
        offer_text = ""
    else:
        offer_text = datum.find("field", {'name':"sbi.offers.offertext"}).text

    if datum.find("field", {'name':"sbi.offers.isonline"}) == None:
        is_online = ""
    else:
        is_online = datum.find("field", {'name':"sbi.offers.isonline"}).text

    if datum.find("field", {'name':"sbi.offers.category"}) == None:
        category = ""
    else:
        category = datum.find("field", {'name':"sbi.offers.category"}).text

    if datum.find("field", {'name':"sbi.offers.startdate"}) == None:
        start_date = ""
    else:
        start_date = datum.find("field", {'name':"sbi.offers.startdate"}).text

    if datum.find("field", {'name':"sbi.offers.enddate"}) == None:
        end_date = ""
    else:
